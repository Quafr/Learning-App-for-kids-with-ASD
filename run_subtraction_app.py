    ...
    'kinesthetic': [
        {
            ...
            'html_file': 'tasks/subtraction_kinesthetic_1.html'
        },
        {
            ...
            'html_file': 'tasks/subtraction_kinesthetic_2.html'
        },
        {
            ...
            'html_file': 'tasks/subtraction_kinesthetic_3.html'
        },
        {
            ...
            'html_file': 'tasks/subtraction_kinesthetic_4.html'
        },
        {
            ...
            'html_file': 'tasks/subtraction_kinesthetic_5.html'
        }
    ]
}
...
def submit_practice_answer(session_id):
    ...
    if session_info['question_index'] >= len(session_info['practice_questions']):
        # Session completed; save to CSV
        save_subtraction_practice_session(session_info)
...
def save_subtraction_practice_session(session_info):
    """Save subtraction practice session data to CSV"""
    if not session_info or 'session_data' not in session_info or not session_info['session_data']:
        return
    
    learner_id = session_info['learner_id']
    log_file = f"subtraction_practice_log{learner_id}.csv"
    file_exists = os.path.exists(log_file)
    
    try:
        with open(log_file, 'a', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=session_info['session_data'][0].keys())
            if not file_exists:
                writer.writeheader()
            writer.writerows(session_info['session_data'])
        print(f"Saved subtraction practice session data to {log_file}")
    except Exception as e:
        print(f"Error saving subtraction practice session: {e}")
...
@app.route('/kinesthetic/<path:filename>')
def serve_kinesthetic(filename):
    """Serve kinesthetic task HTML files from tasks/ folder"""
    try:
        # Support both tasks/filename and direct filename for backward compatibility
        file_path = filename if os.path.exists(filename) else os.path.join('tasks', filename)
