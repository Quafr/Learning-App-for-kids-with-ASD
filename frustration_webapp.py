...
def _read_log_grouped_by_test(log_file):
    ...
    Returns:
      learner_id (str),
      tests (dict[tuple(int, str), list[dict]]) where key is (test_number, mode) and each item has parsed fields.
    """
    ...
    with open(log_file, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Group by both test_number and mode
            key = (test_number, mode)
            tests[key].append({
                "mode": mode,
                "question": question,
                "response_time": rt,
...
def _existing_tests_in_report(output_file, learner_id):
    """
    Read existing report and return the set of (test_number, mode) tuples already written
...
def compute_frustration_per_test(log_file, output_file):
    """
    For each (test_number, mode) combination found in the learner log, write ONE row per mode to the report.
    Avoids duplicates by skipping (test_number, mode) combinations that are already in the report for that learner.
    """
    ...
    rows = []
    for (test_number, mode) in sorted(tests.keys()):
        if (test_number, mode) in already:
            continue  # skip previously scored test/mode combinations
        entries = tests[(test_number, mode)]
