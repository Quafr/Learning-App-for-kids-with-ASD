...
def internal_error(error):
    ...
    return f"Internal Server Error: {str(error)}<br><br>Check the console for details.", 500
...
def save_session(learner_id, session_data):
    ...
    if not session_data:  # Check if session_data is empty to prevent IndexError
        return  # Exit early if no data to save
...
def submit_answer(session_id):
    ...
    question_start_time = session_info.get('question_start_time')
    
    # Validate that we have a question and start time
    if not current_question:
        return jsonify({'error': 'No question loaded. Please load a question first.'}), 400
    
    if question_start_time is None:
        # If start time is not set, set it now (fallback)
        question_start_time = time.time()
        session_info['question_start_time'] = question_start_time
    ...
    if session_info['question_index'] >= len(questions):
        ...
        if len(session_info['completed_modes']) < 3:
            ...
            # Run preferred mode analyzer after all modes are completed
            try:
                import subprocess
                import sys
                # Run preferred mode analyzer in background
                subprocess.Popen([sys.executable, "preferred_mode_analyzer.py"], 
                              cwd=os.path.dirname(os.path.abspath(__file__)))
                print("Started preferred mode analyzer...")
            except Exception as e:
