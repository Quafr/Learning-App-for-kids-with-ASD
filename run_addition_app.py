    ...
    'kinesthetic': [
        {
            ...
            'answer': 3,
            'task': 'Drag 2 stars ⭐⭐ into the box. Then drag 1 more ⭐. Count them all.',
            'html_file': 'tasks/addition_kinesthetic_1.html'
        },
        {
            'question': 'Move 3 apples 🍎🍎🍎 into the basket. Then add 2 more 🍎🍎. Count them.',
            'answer': 5,
            'task': 'Move 3 apples 🍎🍎🍎 into the basket. Then add 2 more 🍎🍎. Count them.',
            'html_file': 'tasks/addition_kinesthetic_2.html'
        },
        {
...
def submit_practice_answer(session_id):
    ...
    if session_info['question_index'] >= len(session_info['practice_questions']):
        ...
        save_addition_practice_session(session_info)
...
def save_addition_practice_session(session_info):
    """Save addition practice session data to CSV"""
    if not session_info or 'session_data' not in session_info or not session_info['session_data']:
        return
    
    learner_id = session_info['learner_id']
    log_file = f"addition_practice_log{learner_id}.csv"
    ...
    try:
        ...
        print(f"Saved addition practice session data to {log_file}")
    except Exception as e:
        print(f"Error saving addition practice session: {e}")
...
def main():
    ...
    try:
        ...
        def open_browser():
            ...
            url = f"http://localhost:5001?learner_id={learner_id}"
            webbrowser.open(url)
